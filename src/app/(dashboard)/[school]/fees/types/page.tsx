...
const emptyForm = { name: "", description: "", isMonthly: false }
...
export default function FeeTypesPage() {
  ...
  function startEdit(ft: FeeType) {
    setEditingId(ft.id)
    setForm({ name: ft.name, description: ft.description ?? "", isMonthly: ft.isMonthly })
    setShowForm(true)
    setMessage("")
  }
    ...
    <div className="p-4 md:p-6 max-w-3xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">Define fee categories for your school. Set amounts per class in <Link href={`/${params.school}/fees/structure`} className="text-indigo-600 underline">Fee Structure →</Link></p>
        </div>
        {!showForm && (
          <button
      ...
      <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 mb-6 text-sm">
        <p className="font-medium text-indigo-900 mb-2">How fee setup works:</p>
        <ol className="space-y-1 text-indigo-700 list-decimal list-inside">
          <li><strong>Step 1 (here):</strong> Add fee type names — e.g. Admission Fee, Tuition Fee, Transport Fee</li>
          <li><strong>Step 2:</strong> Go to <Link href={`/${params.school}/fees/structure`} className="underline font-medium">Fee Structure</Link> — set amount per class for each fee type</li>
          <li><strong>Step 3:</strong> Go to <Link href={`/${params.school}/fees/collect`} className="underline font-medium">Collect Fee</Link> — amounts auto-fill based on student's class</li>
        </ol>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Admission Fee, Tuition Fee, Transport Fee"
                required
      ...
      {feeTypes.length > 0 && (
        <div className="mt-4">
          <Link
            href={`/${params.school}/fees/structure`}
          ...
          >
            Next: Set Class-wise Amounts in Fee Structure →
          </Link>
        </div>
      )}
    </div>
